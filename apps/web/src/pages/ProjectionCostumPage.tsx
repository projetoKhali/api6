...
import { CustomPredictionRequest, CustomPredictionResponse } from '../schemas/ProjectionCostumSchema';
...
function ProjectionCostumPage() {
    ...
    const tableSchema: Column[] = [
        ...
        { key: 'formatted_Fertilizer', label: 'Fertilizante Previsto', type: 'text' },
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setFormData((prev) => ({
            ...prev,
            [name]: isNaN(Number(value)) || name === 'state' || name === 'crop' || name === 'season'
                ? value
                : Number(value),
        }));
    };
        ...
        <div style={{
            ...
            <div style={{
                ...
                {[
                    ...
                    <div key={name} style={{
                        ...
                        <input
                            type={['state', 'crop', 'season'].includes(name) ? 'text' : 'number'}
                            name={name}
                            value={(formData as any)[name] ?? ''}
                            onChange={handleInputChange}
                            style={{ padding: '5px', width: '80%', borderRadius: '4px', border: '1px solid #ccc', fontFamily: ''}}
                        />
                    </div>
                ))}
            </div>
            ...
            {prediction && (
                <div style={{ marginTop: '20px' }}>
                    <h3>Resultado:</h3>
                    <TableComponent schema={tableSchema} data={prediction} onRowSelect={(row) => console.log('Linha:', row)} />
                </div>
            )}
