...
def test_species_crud(test_mongo):
    ...
    species_collection.update_one(
        {"_id": species_id}, {"$set": {"common_name": "Potato (Updated)"}}
    )
...
def test_event_creation(test_mongo):
    ...
    species_id = species_collection.insert_one(
        {"scientific_name": "Solanum tuberosum", "common_name": "Potato"}
    ).inserted_id
    plot_id = plots_collection.insert_one(
        {
            "area": {"unit_of_measure": "km²", "value": 3.0},
            "coordinates": [55.3051, 130.5089],
        }
