...
from api.service.dashboard_service import get_filtered_yield_data, get_filter

def create_blueprint(db):
    yield_blueprint = Blueprint('yield_dashboard', __name__, url_prefix="/api")

    # Configuração CORS para rotas específicas
    CORS(yield_blueprint, resources={
        r"/api/get_yield_data": {"origins": "*", "methods": ["POST", "OPTIONS"]},
        r"/api/get_filters": {"origins": "*", "methods": ["GET", "OPTIONS"]}
    })

    @yield_blueprint.route("/get_yield_data", methods=["POST", "OPTIONS"])
    def get_yield_data():
    ...
    @yield_blueprint.route("/get_filters", methods=["GET", "OPTIONS"])
    def get_filters():
