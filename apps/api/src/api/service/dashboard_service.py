...
from api.models.dashboard_model import DashboardModel
...
def get_yearly_season_totals(filtered_data: List[Dict]) -> Dict:
    ...
    for item in filtered_data:
        ...
        if season == "Whole Year":
            # Acumula a produção do ano inteiro
            whole_year_data[year] += production
        else:
            # Produção normal por estação
    ...
    result = {
        ...
        # Média apenas dos dados "Whole Year"
        "production_average": [0] * len(sorted_years)
    }
...
def get_production_by_state(filtered_data: List[Dict]) -> List[Dict[str, Union[str, float]]]:
    ...
    for state, stats in state_stats.items():
        efficiency = stats['production'] / \
            stats['area'] if stats['area'] > 0 else 0.0
        result.append({
            "state": state,
...
def get_yearly_crop_statistics(filtered_data: List[Dict]) -> Dict[int, List[Dict]]:
    ...
    for item in filtered_data:
        ...
        year_crop_data[year][crop]['productions'].append(
            item.get('production', 0))
        year_crop_data[year][crop]['areas'].append(item.get('area', 0))
        year_crop_data[year][crop]['fertilizers'].append(
            item.get('fertilizer', 0))
        year_crop_data[year][crop]['pesticides'].append(
            item.get('pesticide', 0))
        year_crop_data[year][crop]['rainfalls'].append(
            item.get('annual_rainfall', 0))
    ...
    for year, crops in year_crop_data.items():
        ...
        for crop_name, stats in crops.items():
            ...
            avg_area = sum(stats['areas']) / \
                len(stats['areas']) if stats['areas'] else 0
            total_fertilizer = sum(stats['fertilizers'])
            total_pesticide = sum(stats['pesticides'])
            avg_rainfall = sum(
                stats['rainfalls']) / len(stats['rainfalls']) if stats['rainfalls'] else 0
